/**
 * Weline_Backend：诚实路由 smoke + 后台真实交互 flow（Wave2/3 假用例整治）
 *
...
const CANDIDATE_ROUTES = ["notificationSubscription","notificationsubscription","notification_subscription","config","contact","userContact","usercontact","user_contact","notificationChannel","notificationchannel","notification_channel","notification","backend","index","dashboard"];
...
moduleDescribe(test, MODULE, 'Weline_Backend 后台流程', () => {
  moduleCase(
    test,
  ...
  moduleCase(
    test,
    { module: MODULE, id: 'BACKEND-FLOW-001' },
    '主入口：搜索/筛选或安全控件真实交互',
    async ({ page }) => {
      await loginAsAdmin(page);
      const { route, fatal } = await openPrimary(page);
